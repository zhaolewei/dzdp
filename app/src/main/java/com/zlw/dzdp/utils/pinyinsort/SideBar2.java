 ...
 */
public class SideBar2 extends TextView {
	...
	private void init(AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.SideBar2);
			scaleTime = typedArray.getInteger(R.styleable.SideBar2_scaleTime, 1);
			scaleItemCount = typedArray.getInteger(R.styleable.SideBar2_scaleItemCount, 6);
		}
	...
	private void drawLetters(float y, int index) {
		...
		if (index == -1) {
			...
			for (int i = 0; i < letters.length; i++) {
				...
				// canvas.drawText(letters[i], (float) (p * (currentY - y1) *
				// (currentY - y2)), singleTextH + itemH * i, scaleTextPaint);
				// }
				// canvas.restore();
				// }
			}
